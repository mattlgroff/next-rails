                    ...
                    <Link href={`/${table}`} key={table}>
                        <div className="mb-4 rounded border-2 border-blue-500 bg-white px-8 pb-8 pt-6 shadow-md">
                            ...
                            <ul className="ml-5 list-disc">
                                {Object.entries(tables[table]).map(([column, details]) => {
                                    const columnDetails = details as ColumnDetail;
                                    const dataType = columnDetails.data_type;
                                    const foreignKey = columnDetails.foreign_key;
                                    return (
                                        <li key={column}>
                                            {column} (
                                            <span className="text-gray-600">
                                                {dataType}
                                                {foreignKey && <span className="text-blue-500 ml-2">FK</span>}
                                            </span>
                                            )
                                        </li>
                                    );
